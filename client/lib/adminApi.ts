...
export default async function adminFetch(path: string, init: RequestInit = {}): Promise<MiniResponse | null> {
  try {
    ...
    if (resource === 'upload' && method === 'POST') {
      ...
      try { bodyObj = init.body ? JSON.parse(init.body as string) : null; } catch {}
      const phpRes = await fetch('/api.php?action=upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(init.headers || {}) },
        body: bodyObj ? JSON.stringify(bodyObj) : init.body,
      });
      const json = await (phpRes.ok ? phpRes.json() : phpRes.text().then(t => { try { return JSON.parse(t); } catch { return { error: t }; } }));
    ...
    if (resource === 'chat-sessions' && method === 'GET') {
      ...
      const sessionList = Object.keys(sessions).map((sid) => ({
        sessionId: sid,
        lastMessageAt: sessions[sid][sessions[sid].length - 1]?.createdAt || null,
    ...
    else parsed = await phpRes.text().then(t => { try { return JSON.parse(t); } catch { return t; } });
